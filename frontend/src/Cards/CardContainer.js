...
// transform ${x} 로 위치 지정
const getStyle = (x, y, width, height, zIndex, rotate) => ({
...
class CardContainer extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextBoard.length === 0 && this.props.userCardBoard.includes(card)) {
            console.log('run nB');
            this.flipCard();
        }
    }
    ...
    render() {
        ...
        const defaultStyle = getStyle(index, index, width, height, 0); // initial coords
            ...
            <div>
                <Motion defaultStyle={{x: 1800, y: 1000}} style={sprungRange}>
                    {(
                        ...
                        <div
                            style={getStyle(x, y, width, height, zIndex, rotate)}
                            className="container"
                        >
                            <Card
                                size={size}
